...
import {kwtRec }		        from "@kw/base/class";
//@formatter:on
...
export abstract class kwParam
{
	...
	valDefault: any = null;
	nVar: kwVarEnum = kwVarEnum.null;


	protected constructor(
    private nType: kwParamEnum,
	  private type: kwParamType )
	{
		//console.log("kwParam::constructor() is called.");
	}

	protected abstract loadSrvc(sSrvc: string): kwXField | null
	...
	loadSrvcs(): boolean
	{
		...
		const srvcIn: kwXField | null = this.loadSrvc(sSrvcIn);
    if (srvcIn === null)
    {
      console.error("kwParam::loadSrvcs() srvcIn is invalid.");
      return false;
    }
    ...
    if (!kwXField.is(srvcIn))
		...
		const srvcOut: kwXField | null = this.loadSrvc(sSrvcOut);
    if (srvcIn === null)
    {
	...
	xExport(rec: kwtRec, recX: kwtRec)
	{
	...
	xImport(rec: kwtRec, recX: kwtRec): boolean
	{
